<!DOCTYPE html>
<html lang="cs">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Knihovna - Seznam knih</title>
</head>
<body>
    <header>
        <h1>Aplikace Knihovna</h1>
        <nav>
            <ul>
                <li><a href="/WA-2026-AG/BooksApp/public/index.php">Seznam knih (Domů)</a></li>
                <li><a href="/WA-2026-AG/BooksApp/public/index.php?url=book/create">Přidat novou knihu</a></li>
            </ul>
        </nav>
    </header>
    <main>
        <h2>Dostupné knihy</h2>

        <p>Zde se brzy objeví seznam knih načtený z databáze.</p>

        </main>

    <footer>
        <p>&copy; WA 2026 - Výukový projekt</p>
    </footer>
</body>
</html