...
<main class="container mx-auto px-6 pb-10 pt-6 flex-grow">
    ...
    <div class="mb-6">
        <h2 class="text-3xl font-light tracking-widest text-slate-400 uppercase">Přidat nové album</h2>
        <p class="text-slate-500 mt-2">Vyplňte údaje a uložte album do databáze.</p>
    </div>
    ...
    <div class="bg-slate-800/50 border border-slate-700 rounded-xl overflow-hidden shadow-2xl backdrop-blur-sm p-8">
        ...
        <form action="<?= BASE_URL ?>/index.php?url=album/store" method="post" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="subcategory" name="subcategory" class="w-full bg-slate-900 border border-slate-600 rounded-lg px-4 py-2 text-slate-200 focus:outline-none focus:border-blue-500">
                        ...
                        <?php if (!empty($subcategories)): foreach ($subcategories as $subcat): ?>
                            <option value="<?= htmlspecialchars($subcat['id']) ?>"><?= htmlspecialchars($subcat['name']) ?></option>
                        <?php endforeach; endif; ?>
                    </select>
                </div>
            ...
            <div>
                ...
                <?php for($i=0; $i<=5; $i++): ?>
                <option value="<?= $i ?>"><?= $i ?> hvězdiček</option>
                <?php endfor; ?>
                </select>
            </div>  
