...
<main class="container mx-auto px-6 pb-10 pt-6 flex-grow">
    ...
    <div class="mb-6 flex justify-between items-end border-b border-slate-700 pb-4">
        <div>
            <h2 class="text-3xl font-light tracking-widest text-slate-400 uppercase">Upravit album</h2>
            <p class="text-slate-500 mt-2">Změňte údaje pro: <strong class="text-slate-300"><?= htmlspecialchars($album['title']) ?></strong></p>
        </div>
        <div class="text-right">
            <span class="text-xs text-slate-500 uppercase tracking-wider font-semibold">ID Záznamu</span>
            <div class="text-2xl font-bold text-emerald-500">#<?= htmlspecialchars($album['id']) ?></div>
        </div>
    </div>
    
    <div class="bg-slate-800/50 border border-slate-700 rounded-xl overflow-hidden shadow-2xl backdrop-blur-sm p-8">
        
        <form action="<?= BASE_URL ?>/index.php?url=album/update/<?= htmlspecialchars($album['id']) ?>" method="post" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="interpret" name="interpret" value="<?= htmlspecialchars($album['interpret'] ?? '') ?>" required class="w-full bg-slate-900 border border-slate-600 rounded-lg px-4 py-2 text-slate-200 focus:outline-none focus:border-blue-500">
                </div>
                
                <div>
                    <label for="catalog_number" class="block text-sm font-medium text-slate-300 mb-1">Katalogové číslo</label>
                    <input type="text" id="catalog_number" name="catalog_number" placeholder="Např. SUP 1113" value="<?= htmlspecialchars($album['catalog_number'] ?? '') ?>" class="w-full bg-slate-900 border border-slate-600 rounded-lg px-4 py-2 text-slate-200 focus:outline-none focus:border-blue-500">
                </div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="2" class="w-full bg-slate-900 border border-slate-600 rounded-lg px-4 py-2 text-slate-200 focus:outline-none focus:border-blue-500"><?= htmlspecialchars($album['description'] ?? '') ?></textarea>
            </div>
